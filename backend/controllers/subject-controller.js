...
const subjectCreate = async (req, res) => {
    try {
        ...
        if (existingSubjectBySubCode) {
            res.send({ message: 'Lo sentimos, este subcódigo debe ser único, ya que ya existe' });
        } else {
...
const allSubjects = async (req, res) => {
    try {
        ...
        if (subjects.length > 0) {
            ...
            res.send({ message: "No se encontraron asignaturas" });
        }
    } catch (err) {
        res.status(500).json(err);
    }
...
const getSubjectDetail = async (req, res) => {
    try {
        ...
        if (subject) {
            ...
            res.send({ message: "No se encontró asignatura" });
        }
    } catch (err) {
        res.status(500).json(err);
    }
